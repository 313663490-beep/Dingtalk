...
def generate_summaries(health_items):
    """为待发送的热搜列表生成有温度、有洞察力的专业概述"""
    items_text = "\n".join([f"{item['rank']}. {item['title']}" for item in health_items])
    prompt = f"""你是资深健康媒体编辑，擅长从热搜话题中提炼出公众真正关心的健康要点。
请为以下每条健康热搜撰写一段“编辑点评式”的概述（80-120字），要求：
1. 结合标题中的信息点，用通俗、有温度的语言解释背后的健康常识或医疗风险。
2. 尝试联系近期健康日、科普热点或公众焦虑点，增加现实关联感。
3. 避免复述标题，而是告诉读者“为什么这个话题值得关注”或“你应该知道什么”。
4. 语气像朋友在分享重要健康提醒，但保持专业准确。
...
示例：
假设输入标题为“从息肉到肠癌仅4步”，
输出：1. 概述：很多人不知道，超过80%的肠癌由息肉演变而来，从息肉到癌变通常需要5-10年。这条热搜提醒我们：主动做一次肠镜，可能就是救自己一命。今天也是世界肠道健康日，别让可以预防的癌症成为遗憾。
...
请输出："""
    payload = {
        "model": "deepseek-chat",
        "messages": [
            {"role": "system", "content": "你是资深健康媒体编辑，擅长写出有温度、有洞察力的健康短评。严格按照格式输出。"},
            {"role": "user", "content": prompt}
        ],
        "temperature": 0.7,      # 适度提高温度，增加语言变化
        "max_tokens": 2000,
        "stream": False
    }
    try:
